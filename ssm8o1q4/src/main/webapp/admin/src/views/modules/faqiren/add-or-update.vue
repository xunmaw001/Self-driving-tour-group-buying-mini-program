...
export default {
  data() {
	...
	xiangpian : false,
      ...
      rules: {
          faqizhanghao: [
                { required: true, message: '发起账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          faqixingming: [
                { required: true, message: '发起姓名不能为空', trigger: 'blur' },
          ],
          xingbie: [
          ],
          youxiang: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `faqiren/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.faqirenCrossAddOrUpdateFlag = false;
						 this.parent.search();
